...
function searchQuotes(quotes, query) {
  ...
  return sampleQuotes(
    quotes.filter((quote) => quote.toLowerCase().includes(normalizedQuery)),
    50
  );
}
...
function sampleQuotes(quotes, limit = 10) {
  ...
  for (let index = shuffledQuotes.length - 1; index > 0; index -= 1) {
  ...
  return shuffledQuotes.slice(0, sampleSize);
}
